...
<head>
    ...
    <style>
        ...
        .status-in_progress { background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); color: #065f46; }
            ...
            <header class="bg-white shadow-sm">
                <nav class="px-4 py-3 flex items-center justify-between">
                    ...
                    <div class="flex-grow mx-2 md:mx-4 max-w-xl relative">
                        ...
                        <input type="text" id="searchInput" class="w-full pl-10 pr-4 py-2 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm md:text-base" placeholder="Cari pekerjaan...">
                    </div>
                    ...
                    <div class="flex items-center space-x-2 md:space-x-4">
                        ...
                        <div class="relative group">
                            <button class="focus:outline-none">
                                <img src="{{ auth()->user()->profile_photo_url ?? 'https://via.placeholder.com/32/4B5563/FFFFFF?text=' . auth()->user()->name[0] }}" alt="Profile" class="rounded-full w-8 h-8">
                            </button>
                            <div class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                <a href="#" class="block px-4 py-2 text-gray-800 hover:bg-gray-100">Profil</a>
                                <a href="{{ route('customer.dashboard.history') }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-100">Riwayat Pesanan</a>
                                <hr>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="block px-4 py-2 text-gray-800 hover:bg-gray-100">Keluar</a>
                                </form>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6" id="jobPostsList">
                            ...
                            <div class="job-card bg-white rounded-xl shadow-lg p-6" data-id="{{ $jobPost->id }}">
                                <div class="flex flex-col lg:flex-row lg:items-center justify-between mb-4">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 mb-1">{{ \Carbon\Carbon::parse($jobPost->created_at)->format('d M Y') }}</p>
                                        <p class="text-sm text-gray-600">{{ Str::limit($jobPost->description, 100) }}</p>
                                    </div>
                                    <div class="mt-4 lg:mt-0 flex flex-col items-end">
                                        <span class="status-badge status-{{ $jobPost->status }} px-3 py-1 rounded-full text-sm font-medium mb-2">
                                            <i class="fas fa-{{ $jobPost->status === 'pending' ? 'clock' : ($jobPost->status === 'in_progress' ? 'check-circle' : 'trophy') }} mr-1"></i>
                                            {{ ucfirst(str_replace('_', ' ', $jobPost->status)) }}
                                        </span>
                                        <span class="text-sm text-gray-500">Budget: Rp {{ number_format($jobPost->budget, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                                <div class="flex gap-3">
                                    <button onclick="showApplicationsModal({{ $jobPost->id }})" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                                        <i class="fas fa-eye mr-2"></i>Lihat Penawaran
                                    </button>
                                    <a href="{{ route('customer.dashboard.post-job') }}" class="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                        ...
                        @empty
                            ...
                            <div id="emptyState" class="text-center py-16">
                                ...
                                <a href="{{ route('customer.dashboard.post-job') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200">
    ...
    <div id="applicationsModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-2xl p-8 max-w-4xl w-full mx-4 overflow-y-auto max-h-[80vh]">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold text-gray-900" id="modalTitle">Penawaran untuk: <span id="jobTitle"></span></h3>
        ...
        <div class="bg-white rounded-2xl p-8 max-w-md mx-4 transform transition-all duration-300">
            <div class="text-center">
                ...
                <div class="flex justify-center space-x-2 mb-6">
                    <button class="rating-star text-3xl text-gray-300 hover:text-yellow-400 transition-colors duration-200" data-rating="1"><i class="fas fa-star"></i></button>
                    <button class="rating-star text-3xl text-gray-300 hover:text-yellow-400 transition-colors duration-200" data-rating="2"><i class="fas fa-star"></i></button>
                    <button class="rating-star text-3xl text-gray-300 hover:text-yellow-400 transition-colors duration-200" data-rating="3"><i class="fas fa-star"></i></button>
                    <button class="rating-star text-3xl text-gray-300 hover:text-yellow-400 transition-colors duration-200" data-rating="4"><i class="fas fa-star"></i></button>
                    <button class="rating-star text-3xl text-gray-300 hover:text-yellow-400 transition-colors duration-200" data-rating="5"><i class="fas fa-star"></i></button>
                </div>
                
                <form id="ratingForm" method="POST">
                    @csrf
                    <input type="hidden" name="rating" id="ratingInput">
                    <textarea name="review" class="w-full p-3 border border-gray-300 rounded-lg mb-4" rows="3" placeholder="Tuliskan review Anda (opsional)"></textarea>
                    <div class="flex gap-3">
                        <button type="button" class="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 py-2 px-4 rounded-lg font-medium transition-colors duration-200" onclick="closeRatingModal()">Batal</button>
                        <button type="submit" class="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white py-2 px-4 rounded-lg font-medium transition-colors duration-200">Kirim Rating</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        async function showApplicationsModal(jobPostId) {
            ...
            try {
                ...
                jobTitle.textContent = data.jobPost.title;
                            ...
                            <div class="proposal-card bg-gray-50 rounded-xl p-6" data-status="${app.status}" data-id="${app.id}">
                                <div class="flex flex-col lg:flex-row lg:items-center justify-between mb-4">
                                    <div class="flex items-start space-x-4">
                                        <img src="${app.mitra.profile_photo_url || 'https://via.placeholder.com/60/3B82F6/FFFFFF?text=' + app.mitra.name[0]}" alt="Mitra" class="w-15 h-15 rounded-full border-2 border-blue-200">
                                        <div>
                                            <h3 class="text-lg font-bold text-gray-900">${app.mitra.name}</h3>
                                    ...
                                    <div class="mt-4 lg:mt-0 flex flex-col items-end">
                                        <span class="status-badge status-${app.status} px-3 py-1 rounded-full text-sm font-medium mb-2">
                                            <i class="fas fa-${app.status === 'pending' ? 'clock' : app.status === 'accepted' ? 'check-circle' : app.status === 'rejected' ? 'times-circle' : 'trophy'} mr-1"></i>
                                            ${app.status.charAt(0).toUpperCase() + app.status.slice(1)}
                                        </span>
                                        <span class="text-2xl font-bold text-gray-900">Rp ${Number(app.bid_amount).toLocaleString('id-ID')}</span>
                                        <span class="text-sm text-gray-500">Estimasi ${Math.ceil((new Date(app.estimated_completion_time) - new Date()) / (1000 * 60 * 60 * 24))} hari</span>
                                    </div>
                                </div>
                                <div class="mb-4">
                ...
                showNotification('Gagal memuat penawaran.', 'error');
            }
        }
